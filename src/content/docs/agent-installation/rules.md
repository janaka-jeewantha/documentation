...
| `marketplace` | No  | Marketplace source to register. Accepts GitHub `owner/repo`, a git URL, a local path, or a URL to a `marketplace.json`. Omit for the built-in Anthropic marketplace. |
...
Placeholders are replaced at evaluation time (case-insensitive match). Any `{{name}}` that doesn't correspond to a resolved input is left as-is.

---

## Complete Example

A rule set that reviews newly opened pull requests:
